...
config: dict = get_config(filename=config_filename)

with open("static/food_items.json", "r") as file:
    food_items: List[dict] = loads(file.read())

logger: Logger = get_logger(name=config["db_name"])

keyboards: Keyboards = Keyboards(
    texts=TEXTS["keyboards"],
    web_app_url=config["web"]["url"].format(route="/")
)
...
async def pre_checkout_query_handler(pre_checkout_query: types.PreCheckoutQuery) -> None:
    order_data: List[dict] = loads(pre_checkout_query.invoice_payload.replace("'", "\""))
...
web_app.middlewares.append(
    web.middleware(f=webapp_error_middleware)
)

web_app.add_routes(routes=webapp_routes)
...
async def main():
    runner: web.BaseRunner = web.AppRunner(app=web_app)
    await runner.setup()
